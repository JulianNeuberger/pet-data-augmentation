...
def test_do_augment():
    document = model.Document(
        ...
        relations=[model.Relation(0, 1, "", [1])],
    )

    step = trafo88.SentenceReordering([document])

    mockito.when(trafo88.SentenceReordering)._get_new_ordering(
        mockito.any(model.Document)
    ).thenReturn([2, 0, 1])

    assert trafo88.SentenceReordering._get_new_ordering(document) == [2, 0, 1]
