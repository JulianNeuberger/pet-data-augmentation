import copy
from augment.trafo40 import FillerWordAugmentation
...
def test_do_augment():
    ...
    trafo1 = FillerWordAugmentation()
    trafo2 = FillerWordAugmentation(
        insert_speaker_phrases=False,
        insert_filler_phrases=False,
        tags=["B-Actor", "B-Activity"],
    )
    ...
    # ACT
    # test1
    mockito.when(trafo40).rand().thenReturn(0.1).thenReturn(0.5).thenReturn(
        0.5
    ).thenReturn(0.5).thenReturn(0.5).thenReturn(0.5).thenReturn(0.1).thenReturn(0.5)
    all = [
        "I think",
        "I believe",
        "I mean",
