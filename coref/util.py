...
from data import PetDocument, PetEntity


def resolve_ner_conflicts(
    document: PetDocument,
    mention_indices: typing.List[int],
    ner_tag_strategy: str,
    verbose: bool = False,
) -> typing.Optional[PetEntity]:
    ner_tags = [document.mentions[m_id].type for m_id in mention_indices]
    if len(set(ner_tags)) == 1:
        return PetEntity(tuple(mention_indices))

    if ner_tag_strategy == "keep":
        # keep all mentions regardless of ner tags
        if verbose:
            print(
                f"Resolved entity would have mixed NER tags at mention level. "
        ...
        mention_indices = [
            i for i in mention_indices if document.mentions[i].type == most_frequent_tag
        ]
        return PetEntity(tuple(mention_indices))
    else:
...
def resolve_remaining_mentions_to_entities(document: PetDocument) -> None:
    """
    ...
    for mention_index, mention in enumerate(document.mentions):
        if mention_index in part_of_entity:
            continue
        document.entities.append(PetEntity((mention_index,)))
