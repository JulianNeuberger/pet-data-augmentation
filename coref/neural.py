...
from data import PetEntity, PetDocument
...
class NeuralCoRefSolver:
    ...
    def __init__(
        self,
        co_referencable_tags: typing.List[str],
        ner_tag_strategy: str = "skip",
        min_cluster_overlap: float = 0.33,
        min_mention_overlap: float = 0.1,
        verbose: bool = False,
    ):
        """
    ...
    def _resolve_single_entity(
    ...
    ) -> typing.Optional[PetEntity]:
        if self._verbose:
            print("neuralcoref found the following cluster:")
            for mention_indices in cluster_token_indices:
                mention_text = " ".join(
                    [document.tokens[i].text for i in mention_indices]
                )
                print(
                    f'"{mention_text}", document level token indicies: {mention_indices}'
                )
            print(
                f"Its tokens are contained in the following predicted mentions (of one of the types {self._tags}:"
            )

        mention_indices = set()
        for mention_token_indices in cluster_token_indices:
            mention_index = self._get_mention_for_token_indices(
        ...
        if overlap < self._min_cluster_overlap:
            if self._verbose:
                print(
                    f"Only resolved {len(mention_indices)} of {len(cluster_token_indices)} predicted mentions "
        ...
        if self._verbose:
            print(
                f"Resolved {len(mention_indices)} of {len(cluster_token_indices)} predicted mentions "
                f"({overlap:.2%} overlap)."
            )

        return util.resolve_ner_conflicts(
            document, mention_indices, self._ner_tag_strategy, verbose=self._verbose
        )

    @staticmethod
    def _get_mention_for_token_indices(
        ...
        :returns: index of the matched PetMention if we could find one, None otherwise
        """
        ...
        if is_above_threshold:
            if verbose:
                print(
                    f"{mention.pretty_dump(document)} with an overlap of {overlap:.2%}"
                )
            return mention_index

    def _get_co_reference_indices(
        ...
        doc = self.nlp(
            spacy.tokens.Doc(self.nlp.vocab, [token.text for token in document.tokens])
        )
        ...
        clusters = [
            ...
            if cluster_id.startswith("coref_clusters")
        ]
